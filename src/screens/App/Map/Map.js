...
const Map = ({
  ...
  const [currentLongitude, setCurrentLongitude] = useState(); //41.599114322572305
  const [currentLatitude, setCurrentLatitude] = useState(); //25.383118707686663
  ...
  useEffect(() => {
    messaging().onMessage(async (remoteMessage) => {
      ...
      PushNotification.localNotification({
        ...
        // soundName: "my_hello.mp3", // (optional) Sound to play when the notification is shown. Value of 'default' plays the default sound. It can be set to a custom sound such as 'android.resource://com.xyz/raw/my_sound'. It will look for the 'my_sound' audio file in 'res/raw' directory and play it. default: 'default' (default sound is played)
    ...
    PushNotification.configure({
      onNotification: (notification) => {
        if (notification.userInteraction == true) {
          ...
          } else if (notification.data.scree_name == "Map123") {
            navigation.navigate("Map");
          }
        }
      },
  ...
  console.log("selectedLanguages", user.language_code, selectedLanguages);
  ...
  return (
    <View style={styles.container}>
      ...
      {parseFloat(currentLatitude) > 0 && parseFloat(currentLongitude) > 0 && (
        <MapView
          ref={mapRef}
